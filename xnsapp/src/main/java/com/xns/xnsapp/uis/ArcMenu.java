...
public class ArcMenu extends ViewGroup implements OnClickListener
{
		...
		if (changed)
		{
			...
			for (int i = 0; i < count - 1; i++)
			{
				...
				if(mPosition==Position.CENTER_BOTTOM){
					cl=(int) (mRadius*Math.cos(Math.PI/4+Math.PI/2/(count-2)*i));
					ct=(int) Math.sqrt(Math.pow(mRadius, 2)-Math.pow(cl, 2));
					//ct=(int) (mRadius*Math.sin(Math.PI/(count-1)*i));
					cl=getMeasuredWidth()/2-cWidth/2-cl+8;
					ct = getMeasuredHeight() -cHeight- ct;
				}
	...
	private void layoutButton()
	{
		...
		case RIGHT_BOTTOM:
			...
			break;
		case CENTER_BOTTOM:
			l = getMeasuredWidth()/2-width/2+6;
			t = getMeasuredHeight() - height+3;
			break;
		}
		Log.i(TAG, l + " , " + t + " , " + (l + width) + " , " + (t + height));
